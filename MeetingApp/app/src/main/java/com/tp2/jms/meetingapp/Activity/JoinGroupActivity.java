    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ref.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                adapter = new ArrayAdapter<String>(JoinGroupActivity.this, R.layout.simple_size_list, groupname);
                listView.setAdapter(adapter);
                listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {

                    @Override
                    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                        ProfileAdapter pa = new ProfileAdapter(JoinGroupActivity.this);
                        Profile profile = pa.getProfile(1);
                        ref.child(groupname.get(position)).child("invitations").child(profile.getName()).setValue(profile);
                        Toast.makeText(JoinGroupActivity.this, "An invitation was sent to the group admin", Toast.LENGTH_LONG).show();
                    }
                });
            ...
            public void onCancelled(FirebaseError firebaseError) {
                Toast.makeText(JoinGroupActivity.this, "Failed to read", Toast.LENGTH_SHORT).show();
                finish();
            }
        });
    }
